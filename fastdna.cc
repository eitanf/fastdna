...
using char_mapping_t =
    std::array<uint32_t, 256>; // Mapping from 8 bits to 4 DNA characters
...
constexpr char_mapping_t letters_of_code() {
  ...
  for (int i = 0; i < 256; i++) {
    ...
    const uint32_t decoded = char_of[uint8_t(bits << 0) >> 6] << 0 |
...
void translate(char *stream, fsize_t count, char from, char to) {
...
char *decode(const char *in, char *out, fsize_t remaining_bases) {
  if (g_config.use_pext && remaining_bases >= fsize_t(BPW)) {
    ...
    vpmask_translate(out - bases, bases, 'E', 'T');
